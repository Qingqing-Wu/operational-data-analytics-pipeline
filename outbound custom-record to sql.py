...
def table_has_column(conn, table: str, col: str) -> bool:
    sql = f"SHOW COLUMNS FROM `{table}` LIKE %s"
    with conn.cursor() as cur:
        cur.execute(sql, (col,))
        return cur.fetchone() is not None


def add_column_if_missing(conn, table: str, col: str):
    if table_has_column(conn, table, col):
        return
    sql = f"ALTER TABLE `{table}` ADD COLUMN `{col}` TEXT NULL"
    with conn.cursor() as cur:
        cur.execute(sql)
    conn.commit()
...
def create_main_table_if_not_exists(conn):
    ...
    add_column_if_missing(conn, MAIN_TABLE, ASSEMBLY_DIRECTION_COL)


def compute_hash_id(df: pd.DataFrame) -> pd.Series:
    """
    kind of primary key: source_file + source_row 防撞
    """
    def norm(v) -> str:
        if pd.isna(v):
            return ""
    ...
    def make(row):
        key = (
            f"{norm(row.get('Task order code'))}_"
            f"{norm(row.get('Outbound SN'))}_"
            f"{norm(row.get('UPC/Product Name'))}_"
        ...
        try:
            ...
            if MOVE_FILES:
                try:
                    ...
                    print(f"⚠️ Imported but could not move file (close Excel/preview): {filename}")
